...
public class Container {
	...
	public static Container getContanier()
			throws LocalAllInterfacesUnfoundException,
			NoLocalInterfaceIsImplemented {
		if (contanier == null)
	...
	private Container() throws LocalAllInterfacesUnfoundException,
			NoLocalInterfaceIsImplemented {
	...
	private void fillInterfaceToClass()
		...
		Set<Class<?>> classes = AnnotationsHelper
				.getClassesAnnotatedWith(Stateless.class);
		classes.addAll(AnnotationsHelper
				.getClassesAnnotatedWith(Singleton.class));
		for (Class<?> ejbClasse : classes) {
			...
			if (interfaces.length > 0) {
				...
				for (Class<?> ejbInterface : interfaces) {
					if (AnnotationsHelper.isAnnotatedWith(ejbInterface,
							Local.class)) {
	...
	private void handleEjbAnnotation(Object client) throws EjbInjectionException, EjbClassNotFoundException, PostConstructInvokationException, ProxyMakingException {
	...
	private void invokePostConstructMethods(Object bean) throws PostConstructInvokationException {
		Logger.log("Invoking the post construct methods");
		Set<Method> methods = AnnotationsHelper
				.getMethodsAnnotatedWith(
						((ProxyInterface) bean).getOriginalClass(),
						PostConstruct.class);
		for (Method m : methods) {
			Logger.log("trying to invoke the method '" + m.getName() + "'");
			try {
				Method proxyMethod = bean.getClass().getMethod(m.getName(),
						m.getParameterTypes());
				proxyMethod.invoke(bean, new Object[] {});
				Logger.log("the method '" + m.getName()
	...
	private void invokePreDestroyMethods(Object bean) throws PreDestroyInvokationException {
		Logger.log("Invoking the pre destroy methods");
		Set<Method> methods = AnnotationsHelper.getMethodsAnnotatedWith(
				((ProxyInterface) bean).getOriginalClass(), PreDestroy.class);
		for (Method m : methods) {
			Logger.log("trying to invoke the method '" + m.getName() + "'");
			try {
				Method proxyMethod = bean.getClass().getMethod(m.getName(),
	...
	private Object getProxy(Class<?> clientClass) throws UknwonEjbAnnotation, ProxyMakingException {
		Object bean = null;
		...
		} else if (AnnotationsHelper.isAnnotatedWith(clientClass,
				Singleton.class)) {
	...
	public void removeBean(Object bean) throws PreDestroyInvokationException {
		invokePreDestroyMethods(bean);
		Beans.getInstance().addBeanToClass(bean,
				((ProxyInterface) bean).getOriginalClass());
	}

	private void handlePersistenceAnnotation(Object client) throws EjbInjectionException {
		// get all fields having @PersistenceContext annotation
		Logger.log("Handling @PersistenceContext annotations");
		Set<Field> fields = AnnotationsHelper.getFieldsAnnotatedWith(
				client.getClass(), PersistenceContext.class);
		for (Field field : fields) {
